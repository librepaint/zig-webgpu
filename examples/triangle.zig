...
pub fn main() !void {
    // setup allocator
    var generalPurposeAllocator = std.heap.DebugAllocator(.{}){};
    ...
    // get adapter
    var options = wgpu.WGPURequestAdapterOptions{ .nextInChain = null, .featureLevel = webgpu.FeatureLevel.Core, .powerPreference = webgpu.PowerPreference.NoPreference, .forceFallbackAdapter = webgpu.Bool.False, .backendType = webgpu.BackendType.Vulkan, .compatibleSurface = context._surface };
    ...
    // get device
    var description = wgpu.WGPUDeviceDescriptor{ .nextInChain = null, .label = wgpu.WGPUStringView{ .data = String.usingZigString("My Device").cstring(), .length = String.usingZigString("My Device").len() }, .requiredFeatureCount = 0, .requiredFeatures = null, .requiredLimits = null, .defaultQueue = wgpu.WGPUQueueDescriptor{
        .nextInChain = null,
        .label = wgpu.WGPUStringView{ .data = String.usingZigString("My Queue").cstring(), .length = String.usingZigString("My Queue").len() },
    }, .deviceLostCallbackInfo = wgpu.WGPUDeviceLostCallbackInfo{ .nextInChain = null, .mode = webgpu.CallbackMode.AllowProcessEvents, .callback = onDeviceLost, .userdata1 = null, .userdata2 = null }, .uncapturedErrorCallbackInfo = wgpu.WGPUUncapturedErrorCallbackInfo{ .nextInChain = null, .callback = onDeviceError, .userdata1 = null, .userdata2 = null } };
    ...
    var shaderCodeDesc = wgpu.WGPUShaderSourceWGSL{
        .chain = wgpu.WGPUChainedStruct{ .next = null, .sType = wgpu.WGPUSType_ShaderSourceWGSL },
        .code = wgpu.WGPUStringView{ .data = String.usingZigString(shaderCode).cstring(), .length = String.usingZigString(shaderCode).len() },
    };
    var shaderDesc = wgpu.WGPUShaderModuleDescriptor{ .nextInChain = &shaderCodeDesc.chain };
    const shaderModule: wgpu.WGPUShaderModule = wgpu.wgpuDeviceCreateShaderModule(device._device, &shaderDesc);

    var blendState = wgpu.WGPUBlendState{ .color = wgpu.WGPUBlendComponent{
    ...
    var colorTarget = wgpu.WGPUColorTargetState{ .nextInChain = null, .format = surfaceFormat, .blend = &blendState, .writeMask = wgpu.WGPUColorWriteMask_All };
    var fragmentState = wgpu.WGPUFragmentState{ .nextInChain = null, .module = shaderModule, .entryPoint = wgpu.WGPUStringView{ .data = String.usingZigString("fs_main").cstring(), .length = String.usingZigString("fs_main").len() }, .constantCount = 0, .constants = null, .targetCount = 1, .targets = &colorTarget };
    var pipelineDesc = wgpu.WGPURenderPipelineDescriptor{ .nextInChain = null, .label = wgpu.WGPUStringView{ .data = String.usingZigString("fs_main").cstring(), .length = String.usingZigString("fs_main").len() }, .layout = null, .vertex = wgpu.WGPUVertexState{ .nextInChain = null, .module = shaderModule, .entryPoint = wgpu.WGPUStringView{ .data = String.usingZigString("vs_main").cstring(), .length = String.usingZigString("vs_main").len() }, .constantCount = 0, .constants = null, .bufferCount = 0, .buffers = null }, .primitive = wgpu.WGPUPrimitiveState{ .nextInChain = null, .topology = wgpu.WGPUPrimitiveTopology_TriangleList, .stripIndexFormat = wgpu.WGPUIndexFormat_Undefined, .frontFace = wgpu.WGPUFrontFace_CCW, .cullMode = wgpu.WGPUCullMode_None, .unclippedDepth = webgpu.Bool.False }, .depthStencil = null, .multisample = wgpu.WGPUMultisampleState{
        .nextInChain = null,
        .count = 1,
        .mask = Int.MAX.u32,
    ...
    var renderPassColorAttachment = wgpu.WGPURenderPassColorAttachment{ .nextInChain = null, .view = textureView, .depthSlice = wgpu.WGPU_DEPTH_SLICE_UNDEFINED, .resolveTarget = null, .loadOp = wgpu.WGPULoadOp_Clear, .storeOp = wgpu.WGPUStoreOp_Store, .clearValue = wgpu.WGPUColor{
        .r = 1.0,
        .g = 0.0,
        .b = 0.0,
        .a = 1.0,
    } };
    var renderPass = commandEncoder.beginRenderPass(wgpu.WGPURenderPassDescriptor{ .nextInChain = null, .label = wgpu.WGPUStringView{ .data = String.usingZigString("Surface texture view").cstring(), .length = String.usingZigString("Surface texture view").len() }, .colorAttachmentCount = 1, .colorAttachments = &renderPassColorAttachment, .depthStencilAttachment = null, .occlusionQuerySet = null, .timestampWrites = null });
